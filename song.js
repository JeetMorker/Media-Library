...
// JavaScript code for generating anchor tags for each genre option

// Function to generate anchor tags for each genre option
function generateGenreAnchors() {
    var genreOptions = document.getElementById("genreOptions").getElementsByTagName("option");
    var anchorContainer = document.getElementById("genreAnchors");

    // Clear existing anchor tags
    anchorContainer.innerHTML = "";

    // Generate anchor tags for each genre option
    for (var i = 0; i < genreOptions.length; i++) {
        var genreValue = genreOptions[i].value;
        var genreText = genreOptions[i].textContent;

        var anchor = document.createElement("a");
        anchor.href = "#" + genreValue;
        anchor.textContent = genreText;

        var listItem = document.createElement("li");
        listItem.appendChild(anchor);

        anchorContainer.appendChild(listItem);
    }
}

// Call the function initially and whenever the genre options change
generateGenreAnchors();
document.getElementById("genreOptions").addEventListener("change", generateGenreAnchors);
