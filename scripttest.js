...
export function censorComment(st) {

//copied bad word list from open source https://github.com/MauriceButler/badwords/blob/master/array.js
    ...
    const censoredText = st.replace(badWordsPattern, (matched) => '*'.repeat(matched.length));
    return censoredText;
...
export function loadMovies(document, filteredmovies = movies) { // Takes in filtered movie list
	...
	//Adds movie image to the element
        ...
        grid.appendChild(movieItem); // Adds element to grid
    });
